 ...
 */
public class DictionarySearchHandler extends AbstractSearchHandler {

	@Override
	protected Object doExecute(ExecutionEvent event, VDMEditor editor, DictionaryView view) {
		...
		if(PluginHelper.isEmpty(keyword)) {
			showWarningDialog(Messages.DictionarySearchHandler_0);
			return null;
		}
		...
		if(tab == null) {
			// 辞書が選択されていない場合エラー
			showWarningDialog(Messages.DictionarySearchHandler_1);
			return null;
		}
		...
		if (size == 0) {
			showWarningDialog(Messages.DictionarySearchHandler_2);
			return null;
		}
