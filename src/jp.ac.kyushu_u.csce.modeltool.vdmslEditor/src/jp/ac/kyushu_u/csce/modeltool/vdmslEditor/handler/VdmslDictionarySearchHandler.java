 ...
 */
public class VdmslDictionarySearchHandler extends AbstractVdmslSearchHandler {
	...
	protected Object doExecute(ExecutionEvent event, VdmslEditor editor, DictionaryView view) {
		...
		if(PluginHelper.isEmpty(keyword)) {
			showWarningDialog(Messages.VdmslDictionarySearchHandler_0);
			return null;
		}
		...
		if(tab == null) {
			// 辞書が選択されていない場合エラー
			showWarningDialog(Messages.VdmslDictionarySearchHandler_1);
			return null;
		}
		...
		if (size == 0) {
			showWarningDialog(Messages.VdmslDictionarySearchHandler_2);
			return null;
		}
