 ...
 */
public class VdmrtSearchHandler extends AbstractVdmrtSearchHandler {
	...
	protected Object doExecute(ExecutionEvent event, VdmrtEditor editor, DictionaryView view) {
		...
		if(tab == null) {
			// 辞書が選択されていない場合エラー
			showWarningDialog(Messages.VdmrtSearchHandler_0);
			return null;
		}
...
//		Entry selectEntry = (Entry)selection.getFirstElement();
		...
		if(selectEntry == null) {
			// 見出し語が選択されていない場合エラー
			showWarningDialog(Messages.VdmrtSearchHandler_1);
			return null;
		}
		...
		try {
			...
			// ２回目も見つからなかった場合エラー
			if(region == null) {
				showWarningDialog(Messages.VdmrtSearchHandler_2);
				return null;
			}
